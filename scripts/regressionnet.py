...
'''2019.12.30修改，因为要使用tensorflow的1.x版本，而电脑里面装的是2.x'''
import tensorflow.compat.v1 as tf
tf.disable_v2_behavior()
'''2019.1.1修改需要加上下列code才能够读取到scripts下的alexnet,因为在之后的代码运行中
不是在当前目录下
,否则就会报错的'''
import os
import sys
...
def evaluate_pcp(net, pose_loss_op, test_iterator, summary_writer, dataset_name, tag_prefix='test'):
    ...
    for i, batch in tqdm(enumerate(test_it), total=num_batches):
        ...
        '''2019.12.30日修改，因为原版本是Python2.x版本的，现在改为3.x有问题,2.x能正常运行直接int'''
        # num_joints = tf.cast(num_joints,tf.int32)
